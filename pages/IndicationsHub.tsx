        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-8">
            {filteredPartners.map(partner => (
              <div key={partner.id} className="bg-white rounded-[2.5rem] overflow-hidden border border-slate-50 shadow-sm transition-all pb-8">
                ...
                <div className="px-6 pt-6">
                  ...
                  {/* Endereço */}
                  <div className="flex items-center gap-1.5 text-slate-400 text-[11px] font-bold uppercase tracking-tight mb-3">
                    <MapPin size={12} className="text-purple-400" />
                    {partner.neighborhood ? `${partner.neighborhood}, ` : ''}{partner.city} - {partner.state}
                  </div>

                  {/* Avaliações (AGORA ABAIXO DO ENDEREÇO) */}
                  <div 
                  ...
                  >
                     <div className="flex gap-0.5">
                       {[1,2,3,4,5].map(i => <Star key={i} size={12} className={i <= Math.floor(partner.rating) ? 'fill-amber-400 text-amber-400' : 'text-slate-100'} />)}
                     </div>
                     <span className="text-[10px] font-black text-purple-500 uppercase tracking-widest">{partner.reviewsCount} avaliações</span>
                  </div>

                  {/* Quem indicou */}
                  <div className="flex items-center gap-3 mb-6 bg-slate-50 p-3 rounded-2xl border border-slate-100/50 w-fit">
                    ...
                    <span className="text-[11px] font-bold text-slate-500">Indicado por <span className="text-purple-600">{partner.indicatedBy.name}</span></span>
                  </div>
                  ...
                  {/* Botões com Padding Corrigido (Usando h-14) */}
