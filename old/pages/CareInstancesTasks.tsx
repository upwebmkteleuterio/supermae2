...
import { ArrowLeft, CheckCircle2, Sparkles, Zap, Info } from 'lucide-react';

export const CareInstancesTasks: React.FC = () => {
  const { state, goBack, navigate, toggleCareTask } = useApp();
  ...
  const completedTasksCount = state.careTasks.filter(t => t.completed).length;

  const handleFinishFlow = () => {
    // Só mostramos o modal (recompensa) se o usuário marcou pelo menos 1 item
    if (completedTasksCount > 0) {
      setShowCompletionModal(true);
    } else {
      // Se não marcou nada, apenas finaliza o fluxo e volta para a agenda
      navigate('care_agenda');
    }
  };
      ...
      <div className="px-6 pb-40">
        <div className="bg-slate-50 rounded-[2rem] p-6 mb-8 flex items-start gap-4 border border-slate-100">
          ...
          <p className="text-slate-500 text-[11px] leading-relaxed font-medium">
            Selecione as ações que você se compromete a realizar hoje. O cuidado começa com o primeiro sim para si mesma.
          </p>
        </div>
        ...
        <button 
          ...
          className="w-full bg-[#A855F7] text-white py-5 rounded-[2.5rem] font-black shadow-xl shadow-purple-100 mt-12 active:scale-95 transition-all text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-2"
        >
          <CheckCircle2 className="w-5 h-5" /> Concluir e Salvar
        </button>
      </div>
      ...
      {showCompletionModal && (
        <CareCompletionModal 
          ...
          context={{ 
            ...
            intensity: intensity, 
            tasks: state.careTasks.filter(t => t.completed).map(t => t.text), 
