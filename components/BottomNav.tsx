...
import { LayoutGrid, MessageCircle, Calendar, MessagesSquare, Settings, Heart, LucideIcon } from 'lucide-react';
...
const NAV_ITEMS: NavItemConfig[] = [
  ...
  { id: 'care_list', icon: Heart, label: 'Cuidado', pages: ['care_instances_list', 'care_instances_target', 'care_instances_intensity', 'care_instances_tasks'], target: 'care_instances_target' },
  { id: 'care', icon: Calendar, label: 'Agenda', pages: [
...
export const BottomNav: React.FC = () => {
  ...
  useEffect(() => {
    ...
    itemRefs.current.forEach(ref => { if (ref) resizeObserver.observe(ref); });
    return () => resizeObserver.disconnect();
  }, [activeIndex]);

  useLayoutEffect(() => {
    updatePosition();
    const timers = [50, 300, 600].map(t => setTimeout(updatePosition, t));
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[96%] max-w-md z-50">
      <nav 
        ref={containerRef}
        ...
        {NAV_ITEMS.map((item, index) => {
          ...
          return (
            <button
              ...
              className={`relative z-10 flex items-center gap-2 py-2 px-3 rounded-full transition-colors duration-500 outline-none active:scale-95 ${
            ...
            >
              ...
              {isActive && (
                <span className="text-[10px] font-black whitespace-nowrap tracking-tight animate-in fade-in slide-in-from-left-2">
                  {item.label}
                </span>
              )}
            </button>
